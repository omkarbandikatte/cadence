...
def _get_or_create_live_run(session: Session) -> Run:
    run = session.query(Run).filter_by(mode="LIVE").first()
    if run is not None:
        return run
    run = Run(mode="LIVE", corpus_id="live", seed=0, policy_config_hash="live")
    session.add(run)
    session.flush()
    return run
...
def _already_processed(session: Session, event_id: str) -> bool:
    return (
        ...
        .filter(Ledger.payload["webhook_event_id"].astext == event_id)
        .first()
        is not None
    )
...
async def razorpay_webhook(
    ...
    if not verify_signature(raw_body, x_razorpay_signature, secret):
        run = _get_or_create_live_run(db)
        ledger.record(
            db,
    ...
    if _already_processed(db, event.event_id):
        db.commit()
